...
class BiLSTM_model(nn.Module):
    def __init__(self):
        ...
        self.CNN1 = nn.Conv1d(in_channels=1, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.CNN2 = nn.Conv1d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1)
        # self.maxpool = nn.MaxPool1d(kernel_size=2)
        # self.flattten = nn.Flatten()
        self.BiLSTM1 = nn.LSTM(64, 200, bidirectional=True, batch_first=True)
        self.relu = nn.ReLU()
        self.BiLSTM2 = nn.LSTM(400, 100, bidirectional=True, batch_first=True)
